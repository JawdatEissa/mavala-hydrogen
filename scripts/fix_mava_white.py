"""
Fix MAVA-WHITE image specifically
"""
import json
import requests
from pathlib import Path
from PIL import Image
import numpy as np
import sys

if sys.platform == 'win32':
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')

GREY_RGB = (245, 245, 245)
WHITE_THRESHOLD = 240

def download_and_process():
    script_dir = Path(__file__).parent
    project_root = script_dir.parent
    
    json_path = project_root / 'scraped_data' / 'all_products_new.json'
    images_dir = project_root / 'mavala-hydrogen' / 'public' / 'images' / 'mava-white'
    
    # Load product data
    with open(json_path, 'r', encoding='utf-8') as f:
        products = json.load(f)
    
    # Find MAVA-WHITE
    product = next((p for p in products if p.get('slug') == 'mava-white'), None)
    
    if not product:
        print("❌ MAVA-WHITE not found in data")
        return
    
    print(f"Found: {product.get('title')}")
    print(f"Images: {product.get('images')}")
    
    # Create folder
    images_dir.mkdir(parents=True, exist_ok=True)
    
    # Download each image
    for idx, url in enumerate(product.get('images', [])):
        if 'squarespace' in url:
            print(f"\n📥 Downloading from Squarespace...")
            print(f"   URL: {url}")
            
            # Remove size constraints
            clean_url = url.split('?')[0]
            
            try:
                response = requests.get(clean_url, timeout=30, headers={
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
                })
                response.raise_for_status()
                
                # Save temp file
                temp_path = images_dir / f'temp_{idx}.png'
                with open(temp_path, 'wb') as f:
                    f.write(response.content)
                
                print(f"   ✅ Downloaded")
                
                # Process image
                img = Image.open(temp_path).convert('RGB')
                data = np.array(img)
                
                # Find white pixels
                white_mask = (data[:,:,0] > WHITE_THRESHOLD) & \
                             (data[:,:,1] > WHITE_THRESHOLD) & \
                             (data[:,:,2] > WHITE_THRESHOLD)
                
                white_percentage = np.sum(white_mask) / (data.shape[0] * data.shape[1]) * 100
                print(f"   🔍 White pixels: {white_percentage:.1f}%")
                
                # Replace white with grey
                data[white_mask] = GREY_RGB
                result = Image.fromarray(data)
                
                # Save as PNG
                output_path = images_dir / f'{idx:02d}_Mava-White.png'
                result.save(output_path, 'PNG', optimize=True)
                
                print(f"   🎨 Saved: {output_path.name}")
                
                # Remove temp file
                temp_path.unlink()
                
            except Exception as e:
                print(f"   ❌ Error: {e}")
    
    print("\n✅ MAVA-WHITE processing complete!")
    print(f"📁 Files saved to: {images_dir}")
    
    # List files
    files = list(images_dir.glob('*.png'))
    print(f"\n📋 Files in folder: {len(files)}")
    for f in files:
        print(f"   - {f.name}")

if __name__ == '__main__':
    download_and_process()

