"""
Process Mavala Scientifique K+ Third Image V3 - Better Centered
================================================================
This script:
1. Crops out the nail/badge area from the upper right
2. Crops out the bottom whitespace
3. Scales up the bottle by 80% total (50% + 30% more)
4. Positions the bottle to the LEFT/CENTER (not right)

Usage:
    python scripts/process_scientifique_k_v3.py
"""
...
def process_image_v3(input_path, output_path):
    """
    Process with better centering and larger size.
    """
    print(f"\n{'='*60}")
    print("Mavala Scientifique K+ Image Processor V3")
    print(f"{'='*60}")
    ...
    try:
        ...
        # Step 3: Scale up by 80% total (1.5 * 1.3 = 1.95x, let's use 1.8 for cleaner math)
        ...
        print(f"   Scaled to: {img_scaled.size[0]}x{img_scaled.size[1]} pixels ({scale_factor*100:.0f}% of cropped)")
        
        # Step 4: Create canvas and position bottle to LEFT/CENTER
        print("\n5. Creating final composition with LEFT positioning...")
        canvas_size = max(img_scaled.size[0], img_scaled.size[1])
        canvas = Image.new('RGB', (canvas_size, canvas_size), GREY_RGB)
        
        # Position bottle more to the LEFT
        # Instead of centering (which puts it in middle), offset it to the left
        x_offset = int((canvas_size - img_scaled.size[0]) * 0.35)  # 35% from left (was 50% center)
        y_offset = (canvas_size - img_scaled.size[1]) // 2  # Keep vertically centered
        
        print(f"   Canvas size: {canvas_size}x{canvas_size}")
        print(f"   Bottle position: x={x_offset}, y={y_offset} (LEFT-biased)")
        ...
        print(f"Bottle is now LARGER and positioned to the LEFT")
        print(f"{'='*60}\n")
...
def main():
    ...
    success = process_image_v3(input_path, output_path)
